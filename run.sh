    ...
    for dataset in $datasets; do
        ...
        for sbuck_size in $sbuck_sizes; do
            for dbuck_size in $dbuck_sizes; do
                for c in $cores; do
                    # command="taskset -c $c ./microbench --keys_file=../datasets/${dataset} --keys_file_type=binary --read=${ratio%:*} --insert=${ratio#*:} --operations_num=100000000 --table_size=200000000 --init_table_ratio=0.5 --thread_num=1 --index=buckindex --memory --bli_sbuck_size=${sbuck_size} --bli_dbuck_size=${dbuck_size} --bli_initial_filled_ratio=0.6  --error_bound=4.8"
...
# taskset -c 1 ./microbench --keys_file=../datasets/books --keys_file_type=binary --read=1 --insert0 --operations_num=100000000 --table_size=200000000 --init_table_ratio=0.5 --thread_num=1 --index=buckindex --memory --bli_sbuck_size=8 --bli_dbuck_size=16384 --bli_initial_filled_ratio=0.6 --error_bound=4.8
